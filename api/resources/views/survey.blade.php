    ...
    <div class="container">
        ...
        <form name="survey" method="POST">
            ...
            <div class="form-group row">
                ...
                <div class="col-10">
                    <input class="form-control" type="text" value="{{$organization->name}}" id="name" disabled>
                </div>
            </div>
            <hr/>
            <div class="form-group row">
                ...
                <div class="col-10">
                    <textarea class="form-control" id="mission_statement" name="mission_statement" rows="7" maxlength="2000"{{$token->used ? ' disabled': ''}}>{{$organization->mission_statement}}</textarea>
                    <h6 class="pull-right" id="count_message"></h6>
                </div>
            </div>
            <hr/>
            <div class="form-group row">
                <label for="social_media" class="col-2 col-form-label">Social Media URLs</label>
                <div class="col-10">
                    Website: <input
                                class="form-control"
                                type="text"
                                value="{{$organization->getMedia('Website') ? $organization->getMedia('Website')->handle : ''}}"
                                name="social_media[website]"
                                placeholder="http://www.myphilly.org"
                                {{$token->used ? ' disabled': ''}}>
                    Instagram: <input
                                class="form-control"
                                type="text"
                                value="{{$organization->getMedia('Instagram') ? $organization->getMedia('Instagram')->handle : ''}}"
                                name="social_media[instagram]"
                                placeholder="https://www.instagram.com/codeforphilly/"
                                {{$token->used ? ' disabled': ''}}>
                    Twitter: <input
                                class="form-control"
                                type="text"
                                value="{{$organization->getMedia('Twitter') ? $organization->getMedia('Twitter')->handle : ''}}"
                                name="social_media[twitter]"
                                placeholder="https://twitter.com/codeforphilly/"
                                {{$token->used ? ' disabled': ''}}>
                    Facebook: <input
                                class="form-control"
                                type="text"
                                value="{{$organization->getMedia('Facebook') ? $organization->getMedia('Facebook')->handle : ''}}"
                                name="social_media[facebook]"
                                placeholder="https://www.facebook.com/groups/code4philly/"
                                {{$token->used ? ' disabled': ''}}>
                </div>
            </div>
            <hr/>
            <div class="form-group row">
                ...
                <div class="col-10">
                    ...
                    @foreach($organization->committees as $committee)
                    ...
                    @if(!$token->used && count($organization->committees) < 10)
